...
pub fn refresh_font_cache(directory: &Path) -> AppResult<()> {
    ...
    if let Ok(entries) = std::fs::read_dir(directory) {
        for entry in entries.flatten() {
            let path = entry.path();
            let supported = matches!(path.extension().and_then(|value| value.to_str()).map(str::to_ascii_lowercase).as_deref(), Some("ttf") | Some("otf"));
            if supported {
                let wide_path = path.as_os_str().encode_wide().chain(Some(0)).collect::<Vec<_>>();
                unsafe { let _ = AddFontResourceExW(wide_path.as_ptr(), FR_PRIVATE, std::ptr::null_mut()); }
            }
        }
    }
    unsafe {
        let _ = SendMessageTimeoutW(HWND_BROADCAST, WM_FONTCHANGE, 0, 0, SMTO_ABORTIFHUNG, 1000, std::ptr::null_mut());
    }
    Ok(())
}
...
#[cfg(target_os = "windows")]
#[link(name = "gdi32")]
extern "system" { fn AddFontResourceExW(path: *const u16, flags: u32, reserved: *mut core::ffi::c_void) -> i32; }
#[cfg(target_os = "windows")]
#[link(name = "user32")]
extern "system" { fn SendMessageTimeoutW(window: isize, message: u32, wparam: usize, lparam: isize, flags: u32, timeout: u32, result: *mut usize) -> isize; }
