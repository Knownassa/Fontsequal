...
impl FontProvider for GoogleFontsProvider {
    ...
    fn label(&self) -> &'static str { "Google" }
    fn kind(&self) -> FontProviderKind { FontProviderKind::Remote }
    fn list_families(&self, connection: &Connection) -> AppResult<Vec<FontFamily>> { Ok(repositories::list_font_families(connection)?.into_iter().filter(|font| matches!(font.source, crate::models::FontSource::Google)).collect()) }
