...
pub fn uninstall_managed_files(files: &[FontFile]) -> AppResult<Vec<FontFile>> {
    ...
    for file in files {
        let path = file.path.as_deref().ok_or_else(|| {
            AppError::new("managed_path_missing", "Managed font record has no path.")
        })?;
...
pub fn validate_managed_path(root: &Path, candidate: &Path) -> AppResult<PathBuf> {
    let root = root.canonicalize().map_err(|_| {
        AppError::new(
            ...
            "Fontsequal managed folder is unavailable.",
        )
    })?;
    let candidate = candidate
        .canonicalize()
        .map_err(|_| AppError::new("font_path_missing", "Managed font file is unavailable."))?;
    if !candidate.starts_with(&root) {
        return Err(AppError::new(
            "unsafe_font_path",
            "Only Fontsequal managed font files may be removed.",
        ));
    }
    Ok(candidate)
}
...
mod tests {
    ...
    fn accepts_file_inside_managed_folder() {
        ...
        assert_eq!(
            validate_managed_path(&root, &file).expect("managed path allowed"),
