...
pub fn install_font(family: &FontFamily, variant_ids: Option<&[String]>) -> AppResult<InstallResult> {
    ...
    for variant_id in variants {
        ...
        match write_new_file(&target, &bytes) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                skipped_files.push(managed_file(family, variant_id, source, &target, sha256_bytes(&bytes)));
            }
...
pub fn installed_records(family: &FontFamily, files: &[FontFile]) -> Vec<InstalledFont> {
    files.iter().map(|file| InstalledFont {
        ...
        style: family.variants.iter().find(|variant| Some(&variant.id) == file.variant_id.as_ref()).map(|variant| variant.style.clone()).unwrap_or(crate::models::FontStyle::Normal),
        weight: family.variants.iter().find(|variant| Some(&variant.id) == file.variant_id.as_ref()).map(|variant| variant.weight).unwrap_or(400),
...
pub fn download_font_file(file: &FontFile) -> AppResult<Vec<u8>> {
    if let Some(path) = &file.path { return Ok(fs::read(path)?); }
    let url = file.url.as_deref().ok_or_else(|| AppError::new("font_file_missing", "Font file has no local path or download URL."))?;
    let response = reqwest::blocking::get(url)?.error_for_status()?;
    let bytes = response.bytes()?.to_vec();
    if bytes.len() > 40 * 1024 * 1024 { return Err(AppError::new("font_file_too_large", "Font file exceeds 40 MB.")); }
    Ok(bytes)
}
...
pub fn managed_font_directory() -> AppResult<PathBuf> {
    ...
    Err(AppError::new("unsupported_platform", "User font installation is unsupported on this platform."))
}
...
fn unique_target_path(directory: &Path, family: &FontFamily, variant_id: &str, source: &FontFile) -> PathBuf {
    let extension = match source.format { FontFileFormat::Otf => "otf", _ => "ttf" };
    let safe = format!("{}-{}", safe_name(&family.family), safe_name(variant_id));
    directory.join(format!("{safe}.{extension}"))
}

fn managed_file(family: &FontFamily, variant_id: &str, source: &FontFile, path: &Path, checksum: String) -> FontFile {
    let file_name = path.file_name().and_then(|name| name.to_str()).unwrap_or("font.ttf").to_string();
    FontFile { id: format!("managed-{}-{}", family.id, safe_name(variant_id)), family_id: family.id.clone(), variant_id: Some(variant_id.to_string()), file_name, format: source.format.clone(), path: Some(path.to_string_lossy().into_owned()), url: None, checksum: Some(checksum), size_bytes: path.metadata().ok().map(|metadata| metadata.len()) }
}
...
fn safe_name(value: &str) -> String { value.chars().map(|character| if character.is_ascii_alphanumeric() { character.to_ascii_lowercase() } else { '-' }).collect::<String>().trim_matches('-').to_string() }
