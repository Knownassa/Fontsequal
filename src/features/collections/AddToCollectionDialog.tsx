...
export function AddToCollectionDialog({ familyId }: { familyId: string }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="sm" variant="outline">Collect</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Add to collection</DialogTitle><DialogDescription>Stored locally on this device.</DialogDescription></DialogHeader><div className="space-y-2">{collections.data?.map((collection) => <Button key={collection.id} className="w-full justify-start" variant="outline" onClick={() => add.mutate(collection.id)}>{collection.name}</Button>)}</div><input className="h-9 w-full rounded-md border bg-surface px-3 text-sm text-foreground" placeholder="New collection" value={name} onChange={(event) => setName(event.target.value)} /><DialogFooter><Button disabled={!name.trim() || create.isPending} onClick={() => create.mutate()}>Create and add</Button></DialogFooter></DialogContent></Dialog>;
}
